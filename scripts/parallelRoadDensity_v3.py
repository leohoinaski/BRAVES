...
INPUTS:
    ...
    prefix = 'highways' for primary roads and 'road' for all road types

OUTPUTS:

    prefix+Density_UF_'+str(IBGE_CODES[pp])+'.csv = csv file with road density by state.
...
#%%
def roadDensCity (shpSC,folder,polUsed,indXUsed,IBGEcod,prefix): 
    ...
    for kk in range(0,shpSC.shape[0]): 
        ...
        print('City number = ' + str(kk) + ' of ' + str(shpSC.shape[0]) +
              '  -'+ shpSC.iloc[kk,0])
        print(folder+'/'+prefix+'City_'+shpSC['CD_GEOCMU'].values[kk]+'.csv')
        #print(folder+'/roadCity_'+shpSC['CD_GEOCMU'].values[kk]+'.csv')
        
        file_path = [filename for filename in os.listdir(folder) if 
                     filename.startswith(prefix+'City_'+shpSC['CD_GEOCMU'].values[kk])]
        print(file_path)
        if len(file_path)>0: 
            print('You have the roadCity file for this city = ' + file_path[0])
            roads = pd.read_csv(folder+'/'+prefix+'City_'+shpSC['CD_GEOCMU'].values[kk]+'.csv')
            ...
            if os.path.isfile(folder+'/roadsUF_'+str(int(IBGEcod))+'.csv'):
                print('Using cliped roads in states')
                roads = pd.read_csv(folder+'/roadsUF_'+str(int(IBGEcod))+'.csv')
                roads = roads.drop(roads.columns[0], axis=1)
                roads['geometry'] = roads['geometry'].map(shapely.wkt.loads)
                roads = gpd.GeoDataFrame(roads, crs="EPSG:4326", geometry=roads['geometry'])
                if prefix=='highways':
                ...
                shpFolder = os.path.normpath(folder +os.sep+os.pardir)
                brSHP = gpd.read_file(shpFolder+"/Brasil.shp")
                ...
                if prefix=='highways':
                    roads = roads[((roads['fclass']=='primary') | (roads['fclass']=='secundary') |
                          ...
                          ((roads['maxspeed']>80) & (roads['maxspeed']<160)) | 
                          (roads['ref'].isnull()==False))] 
                ufSHP = brSHP[brSHP['COD_UF']==IBGEcod]
                ufSHP = ufSHP.dissolve(by='UF')
                ufSHP.crs = "EPSG:4326"
                ufSHP = ufSHP.buffer(0.2, resolution=10)
                print('Cliping roads into State ' + str(IBGEcod))
            ...
            for rr in range(0,roadCity.shape[0]):
                ...
                if mlsTest[0] == 'MULTILINESTRING ' or mlsTest[0] == 'MULTILINESTRING' :
                    ...
                    for lin in range(2,len(mlsTest)):
                        ...
                        mlsTest[lin] = mlsTest[lin].replace(')', '')
                        lineStr= mlsTest[lin].split(' ')
                        if lineStr[-1] == '': # linha 156
                            lineStr= np.array(lineStr[0:len(lineStr)-1]) # linha 157
                        else: # linha 158
                            lineStr= np.array(lineStr) # linha 159
...
def roadDensity (dirPath,outPath,IBGE_CODES,lati,latf,loni,lonf,
                 deltaX,deltaY,roadFileName,roadDensPrefix,prefix):
    ...
    print('===================STARTING roadDensity_v3.py=======================')
    print('')
    ...
    for pp in range (0,np.size(IBGE_CODES)):
        ...
        if shpSC.shape[0] < 2:
            ...
            gridLength = roadDensCity (shpSC,folder,polUsed,indXUsed,IBGE_CODES[pp],prefix)
            ...
            chunk_processes = [pool.apply_async(roadDensCity, 
                                                args=(chunk,folder,polUsed,indXUsed,IBGE_CODES[pp],prefix)) for chunk in cityChunks]                    
            #new section
            pool.close()
            pool.join()  
            pool.terminate()
        ...
        gridLength.to_csv(outPath+'/'+prefix+'Density_'+roadDensPrefix+'UF_'+str(IBGE_CODES[pp])+'.csv')
    return gridLength


#%% Main function MCIP
def roadDensityMCIP (dirPath,outPath,IBGE_CODES,roadFileName,roadDensPrefix,mcipPath,prefix):
...
#%% Main function MCIP
def roadDensityWRF (dirPath,outPath,IBGE_CODES,lati,latf,loni,lonf,
                 deltaX,deltaY,roadFileName,roadDensPrefix,wrfPath,prefix):
    ...
    #dataVar = list(ds.variables.keys())
    y = np.unique(ds['XLAT_C'][:])
    x = np.unique(ds['XLONG_C'][:])
    #x = np.arange(loni, lonf+2*deltaX, deltaX)
    #y = np.arange(lati, latf+2*deltaY, deltaY)
    test_naive = pd.date_range('2019-01-01', '2019-04-07', freq='4H')
    tf = TimezoneFinder(in_memory=True)
