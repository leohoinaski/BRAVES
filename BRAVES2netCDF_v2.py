...
def splitnetCDFfiles(dataEmiss,centerX,xX,yY,year,prefix,outPath,
                     fileId,roadDensPrefix,area,ltz):
    month = 1
    ...
    createNETCDFannual(outPath,name,dataTempo,xX,yY,disvec,area,ltz)
    print(name +' is ready')
...
def BRAVES2netCDF (folder,folderSpec,outPath,years,fileId,roadDensPrefix,typeEmiss):
    ...
    print('===================STARTING BRAVES2netCDF_v2.py=======================')
    conver = 1
    ...
    ltz = np.array(gpdData['LTZ'])
    
    names = ['RoadEmiss','HighwaysEmiss','SecondaryEmiss']
         
    
    for name in names: 
        for year in years:
            file_path = [filename for filename in os.listdir(folder) if 
                         filename.startswith("Merge_"+name+"_BySource_Light_"+typeEmiss+'_'+\
            ...
            #Commercial light
            file_path = [filename for filename in os.listdir(folder) if
                         filename.startswith("Merge_"+name+"_BySource_ComLight_"+typeEmiss+'_'+\
                             roadDensPrefix +'_' + str(year)+'.csv')]
            df = pd.read_csv(folder+'/'+file_path[0])
            ...
            # Motorcycles
            file_path = [filename for filename in os.listdir(folder) if 
                         filename.startswith("Merge_"+name+"_BySource_Motorcycle_"+typeEmiss+'_'+\
                             roadDensPrefix +'_' + str(year)+'.csv')]
            df = pd.read_csv(folder+'/'+file_path[0])
            roadX = gpd.GeoDataFrame(df) 
            roadX['geometry'] = roadX['geometry'].apply(wkt.loads) 
            ...
            prefix = name+'_Motorcycles'
            splitnetCDFfiles(dataEmiss3,centerX,xX,yY,year,prefix,
                             outPath,fileId,roadDensPrefix,area,ltz)
            
            # Heavy
            file_path = [filename for filename in os.listdir(folder) if 
                         filename.startswith("Merge_"+name+"_BySource_Heavy_"+typeEmiss+'_'+\
                             roadDensPrefix +'_' + str(year)+'.csv')]
            df = pd.read_csv(folder+'/'+file_path[0])
            roadX = gpd.GeoDataFrame(df) 
            roadX['geometry'] = roadX['geometry'].apply(wkt.loads) 
