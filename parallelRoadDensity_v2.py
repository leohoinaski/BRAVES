...
def roadDensity (dirPath,outPath,IBGE_CODES,lati,latf,loni,lonf,
    ...
    for pp in range (0,np.size(IBGE_CODES)):
        ...
        if shpSC.shape[0] < 2:
            ...
            gridLength = roadDensCity (shpSC,folder,polUsed,indXUsed,IBGE_CODES[pp])
            ...
            chunk_processes = [pool.apply_async(roadDensCity, 
                                                args=(chunk,folder,polUsed,indXUsed,IBGE_CODES[pp])) for chunk in cityChunks]                    
            #new section
            pool.close()
            pool.join()  
            pool.terminate()
...
def roadDensityWRF (dirPath,outPath,IBGE_CODES,lati,latf,loni,lonf,
                 deltaX,deltaY,roadFileName,roadDensPrefix,wrfPath):
